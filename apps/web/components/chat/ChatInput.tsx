  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if ((message.trim() || uploadedImages.length > 0) && !disabled) {
      ...
      if (uploadedImages.length > 0) {
        const imagePaths = uploadedImages.map((img, index) => `Image #${index + 1} path: ${img.path}`).join('\n');
        ...
        {isDragOver && projectId && preferredCli !== 'cursor' && (
          <div className="absolute inset-0 bg-blue-50/90 dark:bg-blue-900/30 rounded-3xl flex items-center justify-center z-10 border-2 border-dashed border-blue-500">
            <div className="text-center">
              ...
              <div className="text-sm font-medium text-blue-600 dark:text-blue-400">Drop images here</div>
              <div className="text-xs text-blue-500 dark:text-blue-500 mt-1">Supports: JPG, PNG, GIF, WEBP</div>
            </div>
          </div>
        )}
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <div className="mode-dropdown-container relative">
              <button
                ...
                title={
                  mode === 'act'
                    ? 'Act Mode: AI can modify code and create/delete files'
              ...
              >
                ...
                <svg
                  ...
                  className={`shrink-0 transition-transform ${showModeDropdown ? 'rotate-180' : ''}`}
                  fill="currentColor"
                >
